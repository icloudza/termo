...
extension TextLayoutManager {
    ...
    private func layoutLine(
        ...
        // ⚠️ termo vendored 补丁（上游 0.12.1 ~ main 的 bug，已提 issue/PR）：
        // 原代码这里有一句 `var maxFoundLineWidth = maxFoundLineWidth`，把 inout 参数遮蔽成局部副本，
        // 导致下方算出的行宽写不回 inout → `maxLineWidth` 恒为 0 → 文本视图永远撑不过视口 → 不换行模式无横向滚动。
        // 删除该遮蔽行后，inout 直接生效，横滚恢复正常。详见 [[editor-roadmap]]。
