...
public class CEUndoManager: UndoManager {
    ...
    /// [termo vendored 补丁] 空闲打断：让下一次输入起一个新的撤销组，使快速连续打字不致塌成单个撤销步
    /// （由 Termo 侧在停止输入约 0.35s 后调用，仿 Xcode 的细粒度撤销）。
    /// 仅在非库级逻辑分组（`isGrouping == false`）时生效，绝不打断缩进/注释/移动行等原子分组。
    public func breakTypingGroup() {
        guard !isGrouping else { return }
        shouldBreakNextGroup = true
    }
